import { aG as computed, aH as unref } from "./vendor.e1f182eb.js";
const useMD5 = function(string) {
...
const USER_PLACEHOLDER = "/logo.png";
const getAvatarUrlFromHash = (hash, size) => `https://www.gravatar.com/avatar/${hash ? hash : ""}?s=${size}`;
const useAvatar = (email, size = 64) => {
  const hash = computed(() => {
    const localEmail = unref(email);
    return localEmail ? useMD5(localEmail) : void 0;
  });
  const src = computed(() => hash.value ? getAvatarUrlFromHash(hash.value, size) : USER_PLACEHOLDER);
  return {
    src
  };
};
